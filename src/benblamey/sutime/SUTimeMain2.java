...
/**
 * Main program for testing SUTime <br> Processing a text string:
 * <pre>
 * -in.type TEXT
 * -date YYYY-MM-dd
 * -i &lt;text&gt;
...
//    pipeline.addAnnotator(new QuantifiableEntityNormalizingAnnotator(false, false));
    ...
    enum InputType {
        ...
        TEXTFILE, TEXT, TIMEBANK_CSV, TEMPEVAL2
    }
...
//        String text = "16 ____ Summer 2010";
//        PrintWriter pw = (out != null) ? IOUtils.getPrintWriter(out) : new PrintWriter(System.out);
      ...
      //  pw.println("<html>");
       // pw.println("<body>");
        ...
        //for (int i = 0; i < split.length; i++) {
            ...
            //String line = text;//split[i];
            //line = line.substring(line.indexOf("___") + 5);

            // Escape HTML entities in the incoming string.
            text = StringEscapeUtils.escapeHtml4(text);
            
            ProcessTextResult ptr = new ProcessTextResult();
            ptr.textAreaText = text;
          ...
          //  pw.println(line);
...

            {
                ...
                for (CoreMap timexannotation : timexes) {
                    ...
                    Object characterOffsetEnd = timexannotation.get(CoreAnnotations.CharacterOffsetEndAnnotation.class);
                }
               ...
               // Collections.reverse(timexList);
                ...
                for (ValuedInterval<CoreMap, Integer> vi : timexList) {
                    ...
                    newText.append("<span class=\"highlight\">");
                    ...
                    System.out.println(characterOffsetStart +" "+characterOffsetEnd);
                }
                ...
                ptr.highlightedHtml = newText.toString();
                
                ptr.highlightedHtml = ptr.highlightedHtml.replaceAll("\\r?\\n", "<br/>\n");
                //System.out.println();
                
            //    pw.append(newText.toString());
            }
            ...
            //ptr.highlightedHtml.replaceAll("", replacement, )            

           // ptr.highlightedHtml.r, newChar)
...
//
//            {
//                //Annotation annotation = textToAnnotation(pipeline, line, date);
...
//
//
//                    Node namedItem = node.getAttributes().getNamedItem("X-GNUPlot-Function");
//
//
//                    if (namedItem != null) {
//                        String gnuPlotFunction = namedItem.getNodeValue();
//
//                        UserContext uc = new UserContext("tempex"); // dummy user.
//                        Gnuplot gnuplot = new Gnuplot(uc);
//                        gnuplot.plots.add(gnuPlotFunction + " title \'graph" + Integer.toString(gnuplot.plots.size()) + "\'");
...
//
        ...
        {
            UserContext uc = new UserContext("test"); // dummy user.
            Gnuplot gnuplot = new Gnuplot(uc);
            gnuplot.AddDemoPlots();
            gnuplot.ExportGraph();
        ...
        props.put("pos.model", "C:\\work\\code\\3rd_Ben\\stanford_nlp/src/edu/stanford/nlp/models/pos-tagger/english-left3words/english-left3words-distsim.tagger");
        ...
        String out = "C:/work/data/output/tempex/sutime.htm";//props.getProperty("o");
       ...
       // processText(pipeline, in, pw, date);
//        break;
//      case TEXTFILE:
//        pipeline = getPipeline(props, true);
